...
# def clean_text(text):
#     # Remove characters that break FFmpeg filters
#     text = text.replace("'", "").replace('"', '')
#     text = text.replace(":", "\\:")  # escape colon
#     text = text.replace("\n", " ")
#     return text.strip()
...
# s   afe_text = clean_text(script_text)
...
# ========================
def generate_script():
    """Generate motivational script using DeepSeek (OpenRouter)."""
    url = "https://openrouter.ai/api/v1/chat/completions"
    headers = {
        "Authorization": f"Bearer {os.getenv('OPENROUTER_API_KEY')}",
        "Content-Type": "application/json"
    ...
    data = {
        "model": "deepseek/deepseek-chat",
        "messages": [
            {"role": "system", "content": "You are GPT-5, a motivational scriptwriter."},
            {"role": "user", "content": "Write a short motivational script about consistency, self-belief, and progress."}
        ]
    }
    response = requests.post(url, headers=headers, json=data)
    response.raise_for_status()
...
def get_pixabay_videos():
    """Fetch video URLs from Pixabay."""
    url = "https://pixabay.com/api/videos/"
    params = {
        "key": os.getenv("PIXABAY_API_KEY"),
        "q": "inspiration landscape",
        "per_page": 3
    }
    r = requests.get(url, params=params)
    r.raise_for_status()
...
def get_local_music():
    """Pick a random local Mixkit music track."""
    music_folder = "music"
    tracks = [f for f in os.listdir(music_folder) if f.endswith(".mp3")]
    if not tracks:
        raise Exception("No local Mixkit music found in /music.")
...
def generate_final_video(video_file, music_file, script_text, output_file):
    """Overlay clean text and merge video + music."""
    first_line = script_text.split("\n")[0][:80]
